# Case Review — PLR_1955_PA (Critical)

## 1) Case Header
- case_id: CASE-PLR_1955_PA
- player_id: PLR_1955_PA
- state_jurisdiction: PA
- risk_category: CRITICAL
- composite_risk_score: 0.860
- primary_driver: BET_ESCALATION
- review_status: SUBMITTED
- score_calculated_at: 2026-02-24 11:41:27.753000-05:00

## 2) Evidence Summary (What Stands Out)
- Composite score is 0.860 with primary driver `BET_ESCALATION`.
- Component profile: loss_chase=0.935, bet_escalation=1.000, market_drift=0.330, temporal=1.000, gamalyze=0.898.
- Recent intensity: 59 bets / $90,096.19 wagered in 7d; 126 bets / $191,232.11 in 90d.
...
```
- Output summary: 1 row returned for PLR_1955_PA with category `CRITICAL` and composite `0.860`.
...
```
- Output summary: total_bets_7d=59, total_wagered_7d=$90,096.19, avg_bet_7d=$1,527.05, max_bet_7d=$1,972.28
...
FROM ordered;
```
- Output summary: avg_after_loss=$1,827.27, avg_after_win=$382.06, after_loss_ratio=4.78
...
LIMIT 1;
```
- Output summary: assessment_date=2025-12-27, sensitivity_to_loss=100.00, sensitivity_to_reward=78.36, risk_tolerance=89.26, decision_consistency=20.74, version=v3.2.1
...
- Sample-size check: 126 bets in 90d (sufficient for ratio interpretation).
- Model consistency check: driver `BET_ESCALATION` aligns with strongest component direction.
